...
use crate::modes::silence::{
    build_segment_ranges, cut_audio_file_dialect_impl, detect_silence,
    gate_segment_head_tail, repair_cut_segment_silence_impl,
    validate_cut_segments_impl,
};
...
#[derive(Clone, Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct CutValidationResult {
    segment_id: String,
    segment_path: String,
...
#[derive(Clone, Debug, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct RepairCutSilenceResult {
    processed_count: usize,
...
#[derive(Clone, Debug)]
pub(crate) struct SegmentEdgeSilence {
    leading_ms: u64,
    trailing_ms: u64,
    duration_ms: u64,
    all_silence: bool,
...
async fn run_semantic_pipeline(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        crate::modes::semantic::run_semantic_pipeline_impl(
            app,
            input_paths,
            out_dir,
            config,
...
pub(crate) fn build_segment_file_name(
    input: &Path,
    role_hint: Option<&str>,
    topic_id_override: Option<u32>,
...
pub(crate) fn uniq_token() -> String {
    SystemTime::now()
        .duration_since(UNIX_EPOCH)
...
pub(crate) fn probe_audio(path: &Path) -> Result<AudioProbe, String> {
...
pub(crate) fn write_pcm_wav_segment(
    input: &Path,
    output: &Path,
    start_sec: f64,
    duration_sec: f64,
...
pub(crate) fn output_pcm_codec(probe: &AudioProbe) -> &'static str {
...
pub(crate) fn seconds_to_ms(value: f64) -> u64 {
...
pub(crate) fn path_file_name(path: &Path) -> String {
    path.file_name()
        .and_then(|value| value.to_str())
        .unwrap_or_default()
        .to_string()
...
pub(crate) fn path_file_stem(path: &Path) -> String {
    path.file_stem()
        .and_then(|value| value.to_str())
        .unwrap_or_default()
        .to_string()
...
mod tests {
    use super::*;
    // Mode 1 cutting code lives in modes::silence as of 2026-05-14.
    // Pull every name that the legacy tests below used to reach via
    // `super::*` back into scope so the tests don't need to migrate.
    use crate::modes::silence::{
        analyze_segment_edge_silence, build_segment_ranges, cut_audio_file_dialect_impl,
        detect_silence, effective_silence_db, gate_segment_head_tail, push_padded_range,
        push_with_smart_split, repair_cut_segment_silence_impl, split_text_by_ranges,
        trim_segment_head_tail, validate_cut_segment, validate_cut_segments_impl,
        SilenceEvent,
    };
...
// ============================================================
//
// The tray icon is the Cloud Worker's at-a-glance status surface:
//   - left-click toggles the main window
//   - menu shows what the worker is doing right now (task name + stage)
//     and offers 打开窗口/彻底退出
//   - menu-bar title shows a Braille-character spinner + short stage
//     label while a task is in flight, returning to empty when idle
//
// The Windows reviewer build has no Worker mode, no background tasks,
// and no need to live in the tray, so the whole thing compiles out.
...
// ------------------------------------------------------------------
// Spinner frames — generated by `scripts/gen_tray_spinner_icons.py`.
// 12 frames at 30° rotation each; @2x retina pair shipped alongside
// so macOS picks the crisp one. We use the 32x32 (@2x) bytes for
// `set_icon`; macOS NSStatusItem downscales for low-DPI menubars on
// its own, and starting from the higher-res source avoids the muddy-
// edge look of upscaled 16x16.
//
// `set_icon_as_template(true)` is flipped on the static idle icon
// at build time so the spinner frames inherit template behaviour
// (macOS auto-inverts to white in dark mode).
// ------------------------------------------------------------------
#[cfg(not(target_os = "windows"))]
const SPINNER_FRAME_BYTES: &[&[u8]] = &[
    include_bytes!("../icons/spinner/frame_00@2x.png"),
    include_bytes!("../icons/spinner/frame_01@2x.png"),
    include_bytes!("../icons/spinner/frame_02@2x.png"),
    include_bytes!("../icons/spinner/frame_03@2x.png"),
    include_bytes!("../icons/spinner/frame_04@2x.png"),
    include_bytes!("../icons/spinner/frame_05@2x.png"),
    include_bytes!("../icons/spinner/frame_06@2x.png"),
    include_bytes!("../icons/spinner/frame_07@2x.png"),
    include_bytes!("../icons/spinner/frame_08@2x.png"),
    include_bytes!("../icons/spinner/frame_09@2x.png"),
    include_bytes!("../icons/spinner/frame_10@2x.png"),
    include_bytes!("../icons/spinner/frame_11@2x.png"),
];

#[cfg(not(target_os = "windows"))]
const IDLE_ICON_BYTES: &[u8] = include_bytes!("../icons/32x32.png");

/// Per-process tray controller — owns the tray icon handle and the
/// spinner thread, exposes a single `update_for_task` entry point that
/// the cloud event listener calls. Stored as Tauri managed state so the
/// `cloud://status-changed` event handler can find it cheaply.
#[cfg(not(target_os = "windows"))]
pub(crate) struct TrayController {
    tray: tauri::tray::TrayIcon<tauri::Wry>,
    task_item: tauri::menu::MenuItem<tauri::Wry>,
    stage_item: tauri::menu::MenuItem<tauri::Wry>,
    inner: std::sync::Mutex<TrayControllerInner>,
}

#[cfg(not(target_os = "windows"))]
#[derive(Default)]
struct TrayControllerInner {
    /// Cancel flag for the spinner thread. Setting this to `true`
    /// makes the next 120ms tick exit cleanly. Replaced (not reused)
    /// every time the spinner starts again.
    spinner_cancel: Option<Arc<AtomicBool>>,
    /// Join handle is kept only so the thread isn't detached entirely
    /// — we never `.join()`, the thread checks its own cancel flag.
    #[allow(dead_code)]
...
#[cfg(not(target_os = "windows"))]
impl TrayController {
    /// React to a CloudState status change. Pass `Some(CurrentTask)`
    /// for "worker is processing X" and `None` for "back to idle".
    pub(crate) fn update_for_task(&self, task: Option<crate::cloud::CurrentTask>) {
        match task {
            Some(t) => {
                let stage_label = stage_label_zh(&t.stage);
                let display_name = display_task_name(&t.name);
                // Menu items: full task name + readable stage. macOS
                // sometimes ignores `\n` in MenuItem text, so we use
                // two separate items rather than one multi-line.
                let _ = self
                    .task_item
                    .set_text(format!("当前任务: {display_name}"));
                let _ = self.stage_item.set_text(format!("阶段: {stage_label}"));
                let _ = self
                    .tray
                    .set_tooltip(Some(format!("正在处理: {display_name} · {stage_label}")));
                // Keep the spinner running with the current short
                // stage as its tail label. Starting it is idempotent
                // — it cancels the previous thread first.
                self.start_spinner(stage_label);
            }
            None => {
                let _ = self.task_item.set_text("空闲");
                let _ = self.stage_item.set_text("等待新任务…");
                let _ = self
                    .tray
                    .set_tooltip(Some("方言标注 — 云端 Worker (空闲)"));
                self.stop_spinner();
            }
        }
    }

    fn start_spinner(&self, stage_label: String) {
        // Always replace — cancel the previous thread before spawning
        // a new one, otherwise two spinners race on `set_title()` and
        // the menubar flickers.
        self.stop_spinner();
        ...
        let handle = std::thread::spawn(move || {
            // Two visual signals run in lock-step: a 10-frame Braille
            // spinner in the menubar title (text), and a 12-frame icon
            // rotation (graphic). Together they make the tray feel
            // "alive" both for users who glance and those who hover.
            //
            // The two arrays have different lengths so they fall
            // slightly out of phase over time — that's intentional, it
            // looks less robotic.
            const TEXT_FRAMES: &[char] =
                &['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
            // Decode each PNG once up front. `tauri::image::Image::from_bytes`
            // is cheap (it copies into an owned buffer) but doing it
            // every tick would still allocate; do it 12 times here
            // instead and reuse.
            let icons: Vec<tauri::image::Image<'static>> = SPINNER_FRAME_BYTES
                .iter()
                .filter_map(|bytes| tauri::image::Image::from_bytes(bytes).ok())
                .collect();
            let mut idx: usize = 0;
            while !cancel_for_thread.load(Ordering::Relaxed) {
                let text_frame = TEXT_FRAMES[idx % TEXT_FRAMES.len()];
                let title = format!("{text_frame} {stage_label}");
                let _ = tray.set_title(Some(title));
                if !icons.is_empty() {
                    let icon = &icons[idx % icons.len()];
                    // `set_icon` takes Option<Image<'_>>; cloning the
                    // owned image is a no-op pointer bump.
                    let _ = tray.set_icon(Some(icon.clone()));
                }
                idx = idx.wrapping_add(1);
                std::thread::sleep(std::time::Duration::from_millis(120));
            }
            // Clean up: clear the menubar title + restore the idle
            // icon so the tray doesn't freeze mid-rotation.
            let _: tauri::Result<()> = tray.set_title(None::<&str>);
            if let Ok(idle) = tauri::image::Image::from_bytes(IDLE_ICON_BYTES) {
                let _ = tray.set_icon(Some(idle));
            }
        });

        let mut inner = self.inner.lock().unwrap();
        inner.spinner_cancel = Some(cancel);
        inner.spinner_handle = Some(handle);
    }

    fn stop_spinner(&self) {
        let mut inner = self.inner.lock().unwrap();
        if let Some(cancel) = inner.spinner_cancel.take() {
            cancel.store(true, Ordering::Relaxed);
        }
        inner.spinner_handle = None;
        // Belt-and-braces: clear the title + restore idle icon now in
        // case the spinner thread hasn't yet noticed the cancel.
        let _: tauri::Result<()> = self.tray.set_title(None::<&str>);
        if let Ok(idle) = tauri::image::Image::from_bytes(IDLE_ICON_BYTES) {
            let _ = self.tray.set_icon(Some(idle));
        }
    }
}

/// Map internal worker stage strings to short human-readable Chinese
/// labels. The strings come from `cloud.rs::run_pipeline` + the
/// `modes/*::run()` set_stage callbacks; keep this mapping in sync
/// when adding a new stage on either side.
#[cfg(not(target_os = "windows"))]
fn stage_label_zh(stage: &str) -> String {
    match stage {
        "claiming" => "认领任务".into(),
        "downloading" => "下载输入".into(),
        "extracting" => "解压".into(),
        "cutting" => "切割中".into(),
        "semantic-cutting" => "语义切割".into(),
        "recognizing" => "识别中".into(),
        "bundling" => "打包产物".into(),
        "uploading" => "上传产物".into(),
        // Stage strings prefixed `<mode>-init` come from cloud.rs's
        // mode-handoff progress event; surface them as "初始化".
        s if s.ends_with("-init") => "初始化".into(),
        // Empty stage = the worker just claimed but hasn't pushed a
        // stage update yet. Treat as "preparing".
        "" => "准备".into(),
        // Unknown stage — show the raw string so the user can see
        // what happened (and report a bug if it's wrong).
        other => other.to_string(),
    }
}

/// Truncate long task names so the tray menu stays narrow. macOS
/// menubar dropdowns expand to fit, but long task names push other
/// items off-screen; ~32 visible CJK chars is the sweet spot.
#[cfg(not(target_os = "windows"))]
fn display_task_name(name: &str) -> String {
    const MAX_VISIBLE_CHARS: usize = 32;
    let chars: Vec<char> = name.chars().collect();
    if chars.len() <= MAX_VISIBLE_CHARS {
        name.to_string()
    } else {
        let head: String = chars.iter().take(MAX_VISIBLE_CHARS).collect();
        format!("{head}…")
    }
}

#[cfg(not(target_os = "windows"))]
fn build_tray(app: &tauri::AppHandle) -> tauri::Result<Arc<TrayController>> {
    use tauri::{
        image::Image,
    ...
    // Two disabled items at the top show what the worker is doing.
    // They get re-labelled in real time via TrayController; the
    // initial text covers the "no task yet" boot state.
    let task_item = MenuItem::with_id(app, "task_info", "空闲", false, None::<&str>)?;
    let stage_item = MenuItem::with_id(app, "stage_info", "等待新任务…", false, None::<&str>)?;
    ...
    Ok(Arc::new(TrayController {
        ...
        task_item,
        stage_item,
        inner: std::sync::Mutex::new(TrayControllerInner::default()),
    }))
}
    ...
    builder
        ...
        .setup(|_app| {
            #[cfg(not(target_os = "windows"))]
            {
                ...
                // The TrayController handle goes into managed state so
                // the `cloud://status-changed` listener (registered
                // below) can find it without a global static.
                match build_tray(_app.handle()) {
                    Ok(controller) => {
                        _app.manage(controller);

                        // Wire CloudState → TrayController. Every time
                        // a worker pipeline pushes a stage update (via
                        // the `set_stage` closure in `cloud.rs::run_pipeline`,
                        // which writes CloudState.current_task + emits
                        // this event), we re-read the state snapshot
                        // and tell the tray controller to redraw / start
                        // / stop the spinner.
                        let app_for_listener = _app.handle().clone();
                        // `listen_any` lives on the `Listener` trait;
                        // import locally so we don't widen the
                        // crate-level use list.
                        use tauri::Listener as _;
                        _app.listen_any("cloud://status-changed", move |_event| {
                            if let Some(state) =
                                app_for_listener.try_state::<cloud::CloudState>()
                            {
                                if let Some(ctrl) = app_for_listener
                                    .try_state::<std::sync::Arc<TrayController>>()
                                {
                                    let snapshot = state.status();
                                    ctrl.update_for_task(snapshot.current_task);
                                }
                            }
                        });
                    }
